<template>
    <button-events :toggleOpen="isOpenNotification" :buttonEvent="notification" @update:clickOutside="toggle" >
        <div class="relative  z-50  max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:divide-gray-600 dark:bg-gray-700 tranistion ease-in-out duration-300 origin-top-right"
                ...
                <a href="#" class="flex px-4 py-3 border-b hover:bg-gray-100 dark:hover:bg-gray-600 dark:border-gray-600">
                    ...
                    <div class="w-full pl-3">
                        <div class="text-gray-500 font-normal text-sm mb-1.5 dark:text-gray-400">New message from <span
                                class="font-semibold text-gray-900 dark:text-white">Bonnie Green</span>: "Hey, what's up?
...
const emit = defineEmits(['update:notificaiton', 'update:isOpen', 'clickOutside'])
const toggle = () => {
    let count = 0;
    count++;
    console.log('toggle' , count);
   return emit('update:notificaiton')
}
const isOpenNotification = computed(() => props.isOpenNotification)
